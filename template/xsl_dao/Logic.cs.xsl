...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="pageTypeStrName" select="'Logic'" />
  <xsl:variable name="pageTypeEntity" select="'Entity'" />
  <xsl:variable name="pageTypeBiz" select="'Biz'" />
  ...
  <xsl:template match="table">
    ...
    <xsl:value-of select="@namespace"/>.<xsl:value-of select="$pageTypeBiz"/><xsl:text disable-output-escaping="yes">;&#xD;&#xA;&#xD;&#xA;</xsl:text>
  ...
  <xsl:template name="printDefaultMethods_Insert">
    ...
    <xsl:value-of select="/tables/table/@ShowCode"/>
    <xsl:value-of select="$pageTypeBiz"/>
    <xsl:text> </xsl:text>
    ...
    <xsl:text disable-output-escaping="yes">                //初始化Biz类&#xD;&#xA;                </xsl:text>
    <xsl:call-template name="translateHeadLetterToLower">
      <xsl:with-param name="propertyName" select="/tables/table/@ShowCode"></xsl:with-param>
    </xsl:call-template>
    <xsl:value-of select="$pageTypeBiz"/>
    ...
    <xsl:value-of select="$pageTypeBiz"/>
    <xsl:text disable-output-escaping="yes">();&#xD;&#xA;&#xD;&#xA;</xsl:text>
    <xsl:text disable-output-escaping="yes">                //执行Biz操作&#xD;&#xA;                </xsl:text>
    <xsl:call-template name="translateHeadLetterToLower">
      <xsl:with-param name="propertyName" select="/tables/table/@ShowCode"></xsl:with-param>
    </xsl:call-template>
    <xsl:value-of select="$pageTypeBiz"/>
